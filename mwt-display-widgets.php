<?php
/*
Plugin Name: MWT-Display Widgets
Plugin URI: https://www.minwt.com
Description: 小工具顯示/隱藏
Author: minwt
...
<label class="dw_toggle" style="display:inline-block; border:solid 1px #333; padding:5px;">顯示 / 隱藏設定</label>
<div class="dw_collapse" style="display:none;">
   <p>
    <select name="<?php echo esc_attr( $widget->get_field_name('dw_include') ); ?>" id="<?php echo esc_attr( $widget->get_field_id('dw_include') ); ?>" class="widefat">
      <option value="0"><?php _e( '隱藏－於', 'display-widgets' ) ?></option>
      <option value="1" <?php echo selected( $instance['dw_include'], 1 ) ?>><?php _e( '顯示－於', 'display-widgets' ) ?></option>
    </select>
   </p>    

	<div style="height:150px; overflow:auto; border:1px solid #dfdfdf; padding:5px; margin-bottom:5px;">
    <h4 class="dw_toggle" style="cursor:pointer;margin-top:0;"><?php _e( '版面設定', 'display-widgets' ) ?> +/-</h4>
    <div class="dw_collapse" style="display:none;">
    <?php
    ...
    <h4 class="dw_toggle" style="cursor:pointer;"><?php _e( '分類') ?> +/-</h4>
    <div class="dw_collapse" style="display:none;">
        ...
        <label for="<?php echo $widget->get_field_id('cat-all'); ?>"><?php _e( '全部分類', 'display-widgets' ); ?></label></p>
	    <?php
    ...
    <h4 class="dw_toggle" style="cursor:pointer;"><?php _e( '頁面') ?> +/-</h4>
    <div class="dw_collapse" style="display:none;">
    <?php 
    ...
    ?>
		...
		<p><label for="<?php echo esc_attr( $widget->get_field_id('other_ids') ); ?>"><?php _e( '文章ID:123,456,789', 'display-widgets' ) ?>:</label><br>
...
function dw_toggle(){jQuery(this).next('.dw_collapse').slideToggle( "slow");}
</script>
<?php
    }  
    function page_types(){
        $page_types = array(
            'front'     => __( '首頁(頁面)', 'display-widgets' ),
            'home'      => __( '首頁', 'display-widgets' ),
            'archive'   => __( '分類頁'),
            'single'    => __( '文章頁'),
            '404'       => '404',
            'search'    => __( '搜尋頁'),
